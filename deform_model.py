...
class PredictionConvolutions(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        self.loc_conv4_3 = DeformConv2d(512, n_boxes['conv4_3'] * 4, kernel_size=3, padding=1)
        self.loc_conv7 = DeformConv2d(1024, n_boxes['conv7'] * 4, kernel_size=3, padding=1)
        self.loc_conv8_2 = DeformConv2d(512, n_boxes['conv8_2'] * 4, kernel_size=3, padding=1)
        self.loc_conv9_2 = DeformConv2d(256, n_boxes['conv9_2'] * 4, kernel_size=3, padding=1)
...
class SSD300(nn.Module):
    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k):
        ...
        for i in range(batch_size):
            ...
            # print(max_scores.size(), best_label.size())
            # print(predicted_scores[i][0:2])
            # exit()
            ...
            for c in range(1, self.n_classes):
                ...
                # print(c, predicted_scores[i].size(), class_scores.size())
                ...
                # exit()
                # print('line 468: ', n_above_min_score, self.n_classes)

                if n_above_min_score == 0:
                    continue

                # print('line 473:', len(score_above_min_score), len(class_scores))
                # exit()
                # print('score above min: ', torch.nonzero(score_above_min_score))
                class_scores = class_scores[torch.nonzero(score_above_min_score)].squeeze(
                    dim=1)  # (n_qualified), n_min_score <= 8732

                # print('For each class:', c, n_above_min_score, 'boxes are valid')
                ...
                # print('Arrives 527')
                # print('class decoded: ', class_decoded_locs.size())
                image_boxes.append(class_decoded_locs[torch.nonzero(1 - suppress).squeeze(dim=1)])
                # print(torch.nonzero(1 - suppress))
                # print(class_decoded_locs.size())
                # print(class_decoded_locs[torch.nonzero(1 - suppress).squeeze(dim=1)].size())
                # exit()
                # print(((1 - suppress).sum()).shape)
                # image_labels.append(torch.LongTensor((1 - suppress).sum() * c).to(device))
