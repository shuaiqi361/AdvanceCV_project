...
def detect_image(image_path, model, min_score, max_overlap, top_k, suppress=None):
    ...
    # Forward prop.
    predicted_locs, predicted_scores, predicted_points = model(image.unsqueeze(0))
    ...
    det_points = det_points * original_dims.repeat(1, 9)
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == '--image':
        ...
        model_path = 'checkpoints/my_checkpoint_anchor_rep_new_b32.pth.tar'
        output_folder = 'rep'
        checkpoint = torch.load(model_path)
        start_epoch = checkpoint['epoch'] + 1
        print(model_path)
