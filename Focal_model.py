    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k):
        ...
        for i in range(batch_size):
            ...
            for c in range(1, self.n_classes):
                ...
                class_scores = class_scores[torch.nonzero(score_above_min_score)].squeeze(dim=1)  # (n_qualified), n_min_score <= 8732

                # print('For each class:', c, n_above_min_score, 'boxes are valid')
                # print(class_scores.size())

                class_decoded_locs = decoded_locs[torch.nonzero(score_above_min_score)].squeeze(dim=1)  # (n_qualified, 4)
...
class MultiFocalLoss(nn.Module):
    ...
    def __init__(self, priors_cxcy, threshold=0.5, neg_pos_ratio=5, alpha=1.):
