...
from TopologyPointModel import SSD300RepPoint, RepPointLoss
...
f_log = open('log/rep_train_topolopy_log.txt', 'w')
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch >= 30 and epoch % 30 == 0:
            ...
            if current_mAP > best_mAP:
                save_checkpoint(epoch, model, optimizer, name='checkpoints/my_checkpoint_topo300_b32.pth.tar')
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (images, boxes, labels, _) in enumerate(train_loader):
        ...
        # Forward prop.
        predicted_priors, predicted_locs_init, predicted_locs_refine, predicted_scores, _ = model(images)  # (N, 8732, 4), (N, 8732, n_classes)

        # Loss
        loss = criterion(predicted_priors, predicted_locs_init, predicted_locs_refine,
    ...
    del predicted_priors, predicted_locs_init, predicted_locs_refine, predicted_scores, images, boxes, labels  # free some memory since their histories may be stored
...
def evaluate(test_loader, model):
    ...
    with torch.no_grad():
        ...
        for i, (images, boxes, labels, difficulties) in enumerate(tqdm(test_loader, desc='Evaluating')):
            ...
            _, _, predicted_locs, predicted_scores, rep_points = model(images)
            time_end = time.time()
